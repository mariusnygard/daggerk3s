...
// DeployArgoCD deploys ArgoCD to a K3s cluster
func (m *Daggerk3) DeployArgoCD(
	ctx context.Context,
	// Name of the cluster
	// +optional
	...
	// Create namespace
	_, err = k3s.Kubectl(fmt.Sprintf("create namespace %s", argoCDNS)).Sync(ctx)
	if err != nil {
		return "", fmt.Errorf("failed to create namespace: %w", err)
	}
	...
	// Deploy ArgoCD
	manifestURL := fmt.Sprintf("https://raw.githubusercontent.com/argoproj/argo-cd/%s/manifests/install.yaml", argoCDVersion)
	output, err := k3s.Kubectl(fmt.Sprintf("apply -n %s -f %s", argoCDNS, manifestURL)).Stdout(ctx)
	if err != nil {
		return "", fmt.Errorf("failed to deploy ArgoCD: %w", err)
	}

	return output, nil
}

// GetArgoCDStatus checks the status of ArgoCD deployments in a cluster
func (m *Daggerk3) GetArgoCDStatus(
	ctx context.Context,
	// Name of the cluster
	// +optional
	// +default="test-cluster"
	...
	output, err := k3s.Kubectl(fmt.Sprintf("get pods -n %s", argoCDNS)).Stdout(ctx)
	if err != nil {
		return "", fmt.Errorf("failed to get ArgoCD status: %w", err)
	}
